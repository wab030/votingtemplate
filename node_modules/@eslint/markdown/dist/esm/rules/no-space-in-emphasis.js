...
export default {
    ...
    create(context) {
         ...
         * @returns {void}
         */
        function reportWhitespace({ originalText, checkIndex, highlightStartIndex, highlightEndIndex, removeIndex, nodeStartLine, nodeStartColumn, }) {
            if (whitespacePattern.test(originalText[checkIndex])) {
                const { lineOffset: startLineOffset, columnOffset: startColumnOffset, } = findOffsets(originalText, highlightStartIndex);
                const { lineOffset: endLineOffset, columnOffset: endColumnOffset, } = findOffsets(originalText, highlightEndIndex);
                context.report({
                    loc: {
                        start: {
        ...
        const bufferManager = {
             ...
             */
            addText(node) {
                const start = node.position.start.offset - this.state.startOffset;
